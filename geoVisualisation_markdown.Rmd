...
Neben reinen Shape-Files gibt es in R die Möglichkeit auch realitätsnähere Karten zu zeichnen. Hier wird beispielhaft eine Karte von Deutschland von [OpenStreetMap](https://www.openstreetmap.de) gezeichnet.
...
Im Folgenden werden die Wahlergebnisse der AfD 2017 bei der Bundestagswahl nach Bundesländern implementiert.
...
Im Folgenden wird das durchschnittliche Einkommen und der Stimmenanteil der AfD pro Bundesland bei der Bundestagswahl 2017 in Deutschland dargestellt.
...
ggmap(germany_map) + coord_sf(crs=st_crs(3857)) + geom_sf(data=merged_stimmen, aes(fill= Votes_AfD), inherit.aes = FALSE,alpha=0.7)+
  scale_fill_gradient2("Stimmenanteil der AfD",low= "white", high = "#00008B")  + ggtitle("Stimmenanteil der AfD")

ggmap(germany_map) + coord_sf(crs=st_crs(3857)) + geom_sf(data=merged_einkommen, aes(fill= Nettoeinkommen), inherit.aes = FALSE,alpha=0.7)+
  scale_fill_gradient2("Nettohaushaltseinkommen",low= "white", high = "#00008B") + ggtitle("Durchschnittliches Einkommen")
```

Zum Schluss wird hier das Wahlergebnis der AfD in Abhängigkeit des durchschnittlichen Nettohaushaltseinkommens dargestellt und die Korrelation zwischen den beiden Größen berechnet.

```{r correlate income and votes}
x <- income[,2]
y <- votes_per_state[,2]
plot(x,y,ylab = "Stimmen für die AFD in %", xlab = "Durchschnittliches Hauhaltseinkommen in €",main ="Zusammenhang zwischen Nettohaushaltseinkommen\n und Wahlergebnis der AfD")
cor.test(x,y,method="pearson")
```

Es besteht eine deutlich negative Korrelation zwischen dem durchschnittlichen Nettohaushaltseinkommen und dem AfD Wahlergebnis. 